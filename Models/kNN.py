...
def earlyprediction(X_train, X_valid, y_valid, X_fulltest, y_fulltest, leng):
  ...
  for i in leng:
    ...
    model = KNN(contamination = 0.026, n_neighbors = 3, method = 'mean')         
    model.fit(X_train)
    final_result = {}
    final_result['parameters'] = {'length = {}, contamination = 0.026, n_neighbors = 3, method = ''mean'''.format(i)}
  ...
  df.to_csv(path + "kNN_EARLY_RESULTS.csv", index = True, sep = ';')
